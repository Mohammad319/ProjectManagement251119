// wwwroot/js/resizeHelper.js
window.resizableDiv = (function () {
    function clamp(v, min, max) { return Math.max(min, Math.min(max, v)); }
    function ensureRelative(el) {
        if (getComputedStyle(el).position === 'static') el.style.position = 'relative';
    }
    return {
        getWidth(el) { return el ? (el.clientWidth || el.getBoundingClientRect().width || 0) : 0; },
        getHeight(el) { return el ? (el.clientHeight || el.getBoundingClientRect().height || 0) : 0; },

        // رأسي (Top/Bottom) بخيار live
        startVerticalGhostDrag(containerEl, splitterEl, dotNetHelper, opts) {
            ...
            let currentY = startY, rafId = null;
            ...
            function frame() {
                ...
                lastValue = v;
                if (live) containerEl.style.setProperty('--top-h', v + 'px');
                else ghost.style.transform = `translateY(${startTopInContainer + dy}px)`;
            }
        ...
        startHorizontalGhostDrag(containerEl, splitterEl, dotNetHelper, opts) {
            ...
            let currentX = startX, rafId = null;
            ...
            function frame() {
                ...
                lastValue = v;
                if (live) containerEl.style.setProperty('--left-w', v + 'px');
                else ghost.style.transform = `translateX(${startLeftInContainer + dx}px)`;
            }
