...
{
    public sealed class CompensationEntity : AuditableEntity<int>, IListOrderDTO
    {
        ...
        private static string NormalizeName(string? value)
        {
            ...
            if (string.IsNullOrWhiteSpace(trimmed))
                throw new ValidationException("Compensation name is required.");
            return trimmed;
        }
        ...
        private static string NormalizeColor(string? value)
        {
            var trimmed = value?.Trim();
            if (string.IsNullOrWhiteSpace(trimmed))
                throw new ValidationException("Compensation color is required.");
            if (trimmed.Length != FieldLengths.ColorHex || trimmed[0] != '#' || !trimmed[1..].All(Uri.IsHexDigit))
                throw new ValidationException("Compensation color must be a valid HEX value like #00ff00.");
