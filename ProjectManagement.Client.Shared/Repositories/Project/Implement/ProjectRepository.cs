    ...
    public class ProjectRepository(HTTPRepository _httpRepository) : IProjectRepository
    {
        ...
        public async Task<List<ListProjectMVVM>> GetOtherDepartmentAsync(Guid folderId, bool includeArchived = false) =>
            (await _httpRepository.GetAsync<List<ListProjectDTO>>(ProjectsURLBase + URLConst.Project.GetProjectsOtherDepartment + $"/{folderId}?includeArchived={includeArchived}"))
                ...
                .OrderByDescending(x => x.Order)
                .ToList();
        public async Task<GetProjectCalcConfigDTO> GetConfig(int? m, int? con, int? com, int? t, int? st, int? proc = null)
        {
            if (!m.HasValue) m = 0;
            ...
            return await _httpRepository.GetAsync<GetProjectCalcConfigDTO>(ProjectsURLBase + $"config/{m}/{con}/{com}/{t}/{st}/{proc}");
        }
