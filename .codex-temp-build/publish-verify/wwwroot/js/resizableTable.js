...
// Seed saved column widths (per-user, restored from the server) into the
// in-memory map and apply them. Keyed by column id without the leading 'h'
// (e.g. "code"), matching the payload sent by SaveTemplateJs.
window.applySavedColumnWidths = function (widths) {
    if (!widths) return;

    // Seed the in-memory map first so a later initializeResizableColumns()
    // (which calls restoreColumnWidths) applies them even if the table isn't
    // rendered yet at this point.
    Object.keys(widths).forEach(id => {
        const w = widths[id];
        if (w > 0) _pmSavedWidths[id] = w;
    });
