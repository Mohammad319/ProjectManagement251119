        ...
        public async Task<IActionResult> Create([FromBody] PostStorygeDTO post)
        {
            if (post.Type == CalculationItemType.resource)
            {
                if (post.copyType == CopyType.Move)
                    return Ok(await MicroBus.Send(new CutResourceCommand(post.Items, post.ParentID, post.OldCalcID, GetUserId(), GetDepartmentId())));
                if (post.copyType == CopyType.Copy)
                    return Ok(await MicroBus.Send(new CopyResourceCommand(post.Items, post.ParentID, post.OldCalcID, GetUserId(), GetDepartmentId())));
            }
            else if (post.Type == CalculationItemType.task)
            {
                if (post.copyType == CopyType.Move)
                    return Ok(await MicroBus.Send(new CutTaskCommand() { Items = post.Items, OldCalcId = post.OldCalcID, NewNetCalcId = post.NewCalcID, TaskParentID = post.ParentID, IsOH = post.IsOH, UserId = GetUserId(), DepartmentId = GetDepartmentId() }));
                if (post.copyType == CopyType.Copy)
                    return Ok(await MicroBus.Send(new CopyTaskCommand(post.Items, post.ParentID > 0 ? post.ParentID : null, post.OldCalcID, post.NewCalcID, post.IsOH, UserId: GetUserId(), DepartmentId: GetDepartmentId())));
            }
            return BadRequest();
        }
