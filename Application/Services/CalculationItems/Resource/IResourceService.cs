...
//        Task<bool> CutAsync(int ParentTaskId, int sourceCalcId, IReadOnlyList<ResourceTaskItemDTO> items, CancellationToken cancellationToken = default);
...
//        public async Task<bool> CutAsync(int ParentTaskId, int sourceCalcId, IReadOnlyList<ResourceTaskItemDTO> items, CancellationToken cancellationToken = default)
//        {
//            var Parent = await _dataAccess.Tasks.Where(x => x.Id == ParentTaskId).Select(x => new
//            {
//                CalID = x.CalculationId,
...
//                if (Resource == null || Resource.ParentTaskId == ParentTaskId) continue;
...
//                Resource.ParentTaskId = ParentTaskId;
...
//                await notification.SendNotificationAsync(sourceCalcId.ToString(), ObjectTypHub.resource, OperationType.MoveRange, Tuple.Create(ParentTaskId, Entities.Select(x => x.Id)));
//            else
//            {
...
//                await notification.SendNotificationAsync(Parent.CalID.ToString(), ObjectTypHub.resource, OperationType.AddRange, ParentTaskId, ListHub);
//            }
//            return true;
//        }
...
//            var updatedEntity = resourcePost.Parse(resourceData.Original.ParentTaskId);
//            updatedEntity.Id = resourceData.Original.Id;
//            updatedEntity.SortOrder = resourceData.Original.SortOrder;
//            updatedEntity.Metadata.QuantityParam = res.Metadata.QuantityParam;
