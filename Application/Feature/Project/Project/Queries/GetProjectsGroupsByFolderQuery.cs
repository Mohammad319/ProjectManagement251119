...
{
    public sealed record GetProjectsGroupsByFolderQuery(Guid FolderId, bool IsVisible, int UserId, int? DepartmentId)
        ...
        : IRequestHandler<GetProjectsGroupsByFolderQuery, IEnumerable<ListProjectDTO>>
    {
        public Task<IEnumerable<ListProjectDTO>> Handle(GetProjectsGroupsByFolderQuery request, CancellationToken ct)
            => service.GetByFolderAsync(request.FolderId, request.IsVisible, request.UserId, request.DepartmentId, ct);
    }
}
