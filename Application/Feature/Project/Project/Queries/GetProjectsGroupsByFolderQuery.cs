...
{
    public sealed record GetProjectsGroupsByFolderQuery(Guid FolderId, bool IncludeArchived, int UserId, int? DepartmentId, bool IsViewer = false)
        ...
        : IRequestHandler<GetProjectsGroupsByFolderQuery, IEnumerable<ListProjectDTO>>
    {
        public Task<IEnumerable<ListProjectDTO>> Handle(GetProjectsGroupsByFolderQuery request, CancellationToken ct)
            => service.GetByFolderAsync(request.FolderId, request.IncludeArchived, request.UserId, request.DepartmentId, ct, request.IsViewer);
    }
}
